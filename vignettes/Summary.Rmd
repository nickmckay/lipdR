...
## LiPD objects
Let's first look at a single LiPD the Moose Lake dataset from Clegg et al. (2010) that's part of the PAGES2k Temperature compilation.
...
First, let's get a quick summary using the `print()` function, or by just returning the object:
...
*NPM - I think that print should include all of the printed text in summary, just not the tables*
...
*NPM - here too, I think that print should include all of the printed text in summary, just not the tables*

and see the number of datasets included, their archiveTypes and geographic boundaries. 

For more information, we can use `summary()`

```{r}
summary(D)
```
And also get tables that give us some basic information on each of the datasets included. There are few options that customize the output of summary, here we will choose the preferred time units, and limit the number of datasets summarized to 20:
...
Sometimes it is useful to save the table, so it can be sorted and further investigated. Simply change return.table to TRUE and assign an output:"

```{r}
MLSummary <- summary(D, return.table = TRUE)
```
...
*NPM: Let's add a print method to the TS as well, and again, let's do all the text but no table*
*NPM: Could we also add the number datasets included in the TS? just length(unique(ts$datasetId))?*
