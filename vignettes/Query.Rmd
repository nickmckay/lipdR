...
```{r}
queryLipdverse(variable.name = c("δ18O"))
```
...
```{r}
queryLipdverse(variable.name = c("18o"))
```
...
```{r}
queryLipdverse(variable.name = c("δ18O", "d18O"))
```
...
```{r}
queryLipdverse(archive.type = unique(queryTable$archiveType)[c(2,3,5,6,39)])
```
Or we can use the simplification strategy again

```{r}
queryLipdverse(archive.type = c("lake"))
```
...
```{r}
queryLipdverse(pub.info = c("10.1029/2008PA0016718K7NFLXQ1"))
```
...
```{r}
queryLipdverse(continent = "North America")
```

Let's grab just those from Mexico

```{r}
queryLipdverse(country = c("Mexico"))
```
...
```{r}
queryLipdverse(coord = c(0,90,-180,-110))
```
...
```{r}
queryLipdverse(compilation = c("wNAm"), grab.data = FALSE)
```

or multiple compilations

```{r}
queryLipdverse(compilation = c("wnam", "temp12k"), grab.data = FALSE)
```
...
```{r}
D <- queryLipdverse(coord = c(0,90,-180,-110),
               archive.type = c("marine", "ocean"),
               ocean = TRUE,
               interp.details = c("sea@surface", "elNino"),
               interp.vars =  c("SST", "upwelling", "SSS"),
