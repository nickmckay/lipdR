...
#L <- readLipd("C:/Users/dce25/Downloads/KISIMA [KN1A].lpd")
#createTSid()
...
lipd2neotoma <- function(L){
  ...
  if (sum(grepl("paleo", attributes(mtabs1)$names)) < 1 & sum(grepl("chron", attributes(mtabs1)$names)) < 1){
    message("No measurement tables found")
  }
  ...
  if (sum(grepl("paleo", attributes(mtabs1)$names)) > 0){
  ...
  if (sum(grepl("chron", attributes(mtabs1)$names)) > 0){
