...
getGeoNeotoma2 <- function(site){
  ...
  geo$latitude <- sitemeta$lat
  geo$longitude <-  sitemeta$long
...
getChronDataNeotoma2 <- function(site){
  ...
  for(cd in seq_len(nChronData)){
    ...
    for(cc in seq_len(nchronologies)){
      ...
      mt[[cc]]$tableMetadata <- SeuratObject::S4ToList(site@collunits[[cd]]@chronologies@chronologies[[cc]])
    ...
    chronData[[cd]]$chronMetadata <- SeuratObject::S4ToList(site@collunits[[cd]])
    chronData[[cd]]$chronMetadata$datasets <- NULL
    chronData[[cd]]$chronMetadata$samples <- NULL
