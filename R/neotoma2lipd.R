...
getPubNeotoma2 <- function(site){
  ...
  # Fetching publications requires a live Neotoma API call. Degrade gracefully
  # (return NA) if the call fails, e.g. offline or during CRAN checks.
  pubMeta <- tryCatch(
    neotoma2::get_publications(datasetid = site@collunits[[1]]@datasets[[1]]@datasetid),
  ...
  # The neotoma->lipd field-name mapping table (`nc`) must be available; if not,
  # there is nothing to map, so return NA rather than erroring.
  if(!exists("nc")){
    return(NA)
  }
