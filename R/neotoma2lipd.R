...
neotoma2lipd <- function(site){
  ...
  ##############################################
  #check for duplicate sample depths and combine
  ##############################################
  #get sample data from neotoma
  sampData1 <- neotoma2::samples(site)

  #collection units map to paleoData objects
  npaleoData <- length(unique(sampData1$collunitid))

  paleoData <- vector(mode = "list",npaleoData)
  for(cc in seq_len(npaleoData)){

    depths <- unlist(lapply(site@collunits[[cc]]@datasets[[1]]@samples@samples, function(x) x@depth))
    duplicates <- duplicated(depths)


    if(sum(duplicates)>0){
      message("Found duplcate sample depths...combining")
      depthDups <- depths[duplicates]
      for (ii in depthDups){
        sampleIndexDups <- which(depths == ii)
        site@collunits[[cc]]@datasets[[1]]@samples@samples[sampleIndexDups]

        samplesAll <- lapply(site@collunits[[cc]]@datasets[[1]]@samples@samples[sampleIndexDups], function(x) x@datum)
        weldedSampsDF <- do.call(rbind, samplesAll)

        weldedSampsDF <- weldedSampsDF[-c(intersect(which(weldedSampsDF$ecologicalgroup == "LABO"),which(duplicated(weldedSampsDF)))),]

        weldedSampsDF <- weldedSampsDF %>%
          group_by(units, context, element, taxonid, symmetry, taxongroup, elementtype, variablename, ecologicalgroup) %>%
